...
//  }
show_more_show_less(skill_cont,'skill-cont-show-more','<p>Show all skills</p><i class="fa-solid fa-arrow-right font-20"></i>','<p>Show less skills</p><i class="fa-solid fa-arrow-right font-20"></i>',show_all_skills);
})
//Quote->All smart worker once a hard worker
show_all_education.addEventListener("click",()=>{
    show_more_show_less(edu_cont,'edu-cont-show-more','<p>Show all educations</p><i class="fa-solid fa-arrow-right font-20"></i>','<p>Show less educations</p><i class="fa-solid fa-arrow-right font-20"></i>',show_all_education);
})

// edit/update profile.
 let edit_info = document.getElementById("edit-info");
 let edit_profile_ui= document.querySelector(".edit-profile-ui");
 let edit_skill_ui =document.querySelector(".edit-skill-ui");
...
edit_btn.forEach((element)=>{
    element.addEventListener('click',(e)=>{
        console.log(e.target)
            if(e.target.id=='skill-edit-btn' || e.target.id=='skill-edit-img'){
            edit_skill_ui.classList.toggle("disp-none");
            focus_change();
                           }
    })
})
